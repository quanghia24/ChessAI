...
class Board:
    ...
    def get_hash(self):
        # Simple hash: convert all piece positions into a string
        pieces = []
        for row in self.tilemap:
            for tile in row:
                if tile.piece:
                    pieces.append(f"{tile.piece.color}{tile.piece.__class__.__name__}{tile.x}{tile.y}")
        return ''.join(sorted(pieces)) + str(self.turn)
            ...
            for y in range(8):
                if self.piece_at_coords((x, y)) and self.tilemap[x][y].piece.color == self.turn:
                    ...
                    for move in moves:
                        if not self.in_check_after_move((x, y), move, self.tilemap[x][y].piece.color):
                            legal_moves += 1
        if legal_moves == 0 and self.in_check(self.turn):
            return True
        return False
    ...
    def is_capture(self, source, dest):
        dest_tile = self.tilemap[dest[0]][dest[1]]
        source_tile = self.tilemap[source[0]][source[1]]
        return (
            dest_tile.piece is not None and
            source_tile.piece is not None and
            dest_tile.piece.color != source_tile.piece.color
        )
    ...
    def get_attackers(self, x, y, attacker_color, include_self=False):
        """
        Returns a list of all pieces of `attacker_color` that can attack square (x, y).
        If include_self is True, it also considers pieces of the opposite color 
        (e.g., for defense analysis).
        """
        attackers = []
        target_coords = (x, y)
        ...
        for row in self.tilemap:
            for tile in row:
                ...
                if piece is None:
                    continue
                if piece.color == attacker_color or (include_self and piece.color != attacker_color):
                    valid_moves = piece.valid_moves(self)
                    for move in valid_moves:
                        if move == target_coords:
                            attackers.append(piece)
                            break  # No need to check more moves for this piece
    ...
    def print_board_state(self):
        print("\n    A  B  C  D  E  F  G  H")
        print("   ------------------------")
        for y in range(7, -1, -1):  # Print from rank 8 to 1 (top to bottom)
            row = f"{y+1} |"
            for x in range(8):
                piece = self.tilemap[x][y].piece
                if piece:
    ...
    def piece_symbol(self, piece):
        ...
        symbol = symbol_map.get(piece.__class__.__name__, "?")
        return symbol.lower() if piece.color == BLACK else symbol.upper()
